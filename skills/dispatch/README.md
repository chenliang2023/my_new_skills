...
服务器端（CodeG）执行阶段使用的 skills。用户调用，驱动三个 agent（Claude Code / Pi / Google Antigravity）并发执行 ticket。
...
- **[route-agent](./route-agent/SKILL.md)**：根据 ticket 内容自动判断应分派给哪个 agent（Claude Code/Pi/AntiGravity）。
- **[integrate](./integrate/SKILL.md)**：跨 ticket 协调（补充用）。CodeG 的 Merge 流程已处理单任务级别的冲突解决和 git 验证，本 skill 只在跨 ticket 问题时使用。
- **[verify](./verify/SKILL.md)**：全量验证（类型检查、测试、构建、lint），可利用 CodeG 的 preflight command 或手动执行。