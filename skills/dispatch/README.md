...
服务器端（CodeG）执行阶段使用的 skills。用户调用，驱动三个 agent（DeepSeek Harness / Pi / Google Antigravity）并发执行 ticket。
...
- **[route-agent](./route-agent/SKILL.md)**：根据 ticket 内容自动判断应分派给哪个 agent（DeepSeek Harness / Pi / Google Antigravity）。供 `/dispatch` 调用，也可独立运行做路由预检。
